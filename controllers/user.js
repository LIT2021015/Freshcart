...
export const addNewUser = async (req, res) => {
  try {
    ...
    await sendMail(
      email,
      "Freshcart Verification Code",
      `Hey ${name}, your Freshcart verification code is: ${otp}.`
    );
...
export const addressDelete = async (req, res) => {
  try {
    ...
    user.address = user.address.filter(
      (e) => e._id.toString() !== addressID.toString()
    );
    await user.save();
    res.status(200).json({
      success: true,
...
export const addressUpdate = async (req, res) => {
  try {
    ...
    const { street, locality, city, state, country, pincode } = req.body;
    const user = await User.findById(req.user._id);
    user.addressUpdate = user.address.find(
      (e) => e._id.toString() === addressID.toString()
    );
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    await sendMail(
      email,
      "Freshcart Verification Code",
      `Hey ${user.name}, your Freshcart verification code is: ${otp}. If you didn't request for this, please ignore this mail.`
    );
...
export const resetPassword = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Password is changed successfully!" });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
export const cartAddItem = async (req, res) => {
  try {
    ...
    const user = await User.findById(req.user._id);
    const alreadyExist = user.cart.findIndex(
    ...
    user.cart.push({
      product,
      quantity: 1,
      total: product.pprice - product.pdiscount,
    });
    await user.save();
...
export const cartDetails = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Cart Items", cart: user.cart });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
